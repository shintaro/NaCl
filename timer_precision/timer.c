...
static const char* const kTimerMethodId = "timer";
...
static void timer(char* t) {
  ...
  msec = atoi(t);
  gettimeofday(&tv,&tz);
  sprintf(&count[0], "%lf\n", ( (double) tv.tv_sec + (double) tv.tv_usec * 1.e-6 ));
  usleep(msec);
  gettimeofday(&tv,&tz);
  sprintf(&tmp[0], "%lf\n", ( (double) tv.tv_sec + (double) tv.tv_usec * 1.e-6 ));
  strcat(count, tmp);
